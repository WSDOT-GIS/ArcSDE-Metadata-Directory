...
Parameters:
  ...
  externalCss:
    Assign a url of a CSS file to this parameter to have that CSS file referenced in the output HTML.
  jQueryUrl:
    The JavaScript code used by this stylesheet requires jQuery.  You can specify an alternate URL to the jQuery library if you do not want to use the default.
    If you are specifying your own external JS file (via the externalJS parameter) and the script you are specifying does not require jQuery, you can set
    the jQueryUrl parameter to an empty string.
  includeJavaScript:
    Set this value to true to include the inline JavaScript code in the output HTML, or false to omit it.  Note that if the externalJS parameter is given a value,
    the includeJavaScript parameter will be ignored (and no inline JavaScript will be included).
    Defaults to true().
  includeCss:
    Set this value to true if you want to include inline CSS in the output HTML, or false to omit it.  Note that if extnernalCss is given a value, the includeCss
    parameter will be ignored (i.e., no inline CSS).
    Defaults to true().
-->
...
14. Modified August 12, 2011 (ver 2.4) by Jeff Jacobson
    - Modified XSLT so that output HTML will be HTML 5 standards compliant.  (E.g., No duplicate element IDs.)
    ...
    - Added the option for specifying external links to JavaScript and CSS files instead of including them inline.
-->
<xsl:template match="/">
...
<xsl:call-template name="footer"/>
  <xsl:choose>
    <xsl:when test="normalize-space($externalJS) != ''">
      <xsl:if test="normalize-space($jQueryUrl) != ''">
        <xsl:element name="script"><xsl:attribute name="src"><xsl:value-of select="$jQueryUrl"/></xsl:attribute></xsl:element>
      </xsl:if>
      <xsl:element name="script"><xsl:attribute name="src"><xsl:value-of select="$externalJS"/></xsl:attribute></xsl:element>
    </xsl:when>
    ...
    <xsl:for-each select="onlink[normalize-space(.) != '']">
      <xsl:choose>
        <xsl:when test="position() = 1">
          ...
          <b>Data location: </b><xsl:value-of select="."/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:text>, </xsl:text><xsl:value-of select="."/>
        </xsl:otherwise>
